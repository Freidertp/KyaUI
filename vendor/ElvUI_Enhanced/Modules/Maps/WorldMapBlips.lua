...
local BLIP_TEXTURE = [[Interface\Addons\ElvUI_Enhanced\Media\Textures\PartyRaidBlips]]
local DEFAULT_PARTY_ICON = [[Interface\WorldMap\WorldMapPartyIcon]]
local DEFAULT_RAID_ICON = [[Interface\WorldMap\WorldMapRaidIcon]]
...
local function OnShowParty(self)
	...
	if self.class ~= class then
		self.class = class
		local coords = BLIP_TEX_COORDS[class]
		if coords then
			self.icon:SetTexture(BLIP_TEXTURE)
			self.icon:SetTexCoord(coords[1], coords[2], coords[3], coords[4])
		else -- clase custom del cliente (ej. REAPER): punto normal por defecto
			self.icon:SetTexture(DEFAULT_PARTY_ICON)
	...
	if self.class ~= class or self.inParty ~= inParty then
		...
		if coords then
			...
			if inParty then
				...
				self.icon:SetTexCoord(coords[1], coords[2], coords[3] + BLIP_RAID_Y_OFFSET, coords[4] + BLIP_RAID_Y_OFFSET)
			end
		else -- clase custom del cliente (ej. REAPER): punto normal por defecto
			self.icon:SetTexture(DEFAULT_RAID_ICON)
			self.icon:SetTexCoord(0, 1, 0, 1)
		end
	end
end
