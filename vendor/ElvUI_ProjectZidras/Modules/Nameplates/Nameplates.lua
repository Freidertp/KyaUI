...
function ZNP:NAME_PLATE_UNIT_ADDED(_, unit)
	...
	E:Delay(0.05, function()
		if not plate then return end -- guard: sin plate HD (p.ej. Ascension_NamePlates desactivado)
		local frame = plate.UnitFrame
		if not frame then -- Compatibility with VirtualPlates
            frame = plate:GetChildren().UnitFrame
            plate = plate:GetChildren()
        end
		if not (frame and frame.oldHealthBar) then return end -- guard: placa ElvUI aún no construida (cliente HD); se coloreará al mostrarse
